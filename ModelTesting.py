...
for user_col in user_cols:
    ...
    for user_id, hist in groups_by_user:
        ...
        if hists_already_determined or (min_hist_len <= len(hist) * train_frac
            ...
            hist_train_range = [curr_h2_len, curr_h2_len + int(len(hist) * train_frac)]
            curr_h2_len += int(len(hist) * train_frac)
            hists_by_user[user_id] = [hist, hist_train_range]
    ...
    for seed_idx, seed in enumerate(seeds):
        ...
        hists_seed_by_user = {}
        hists_test = {}
        for user_id, item in hists_by_user.items():
            hist, hist_train_range = item
            ...
            hists_seed_by_user[user_id] = [hist_train_and_valid, hist_test_x, hist_test_y, hist_train_range]
        ...
        for inner_seed_idx, inner_seed in enumerate(inner_seeds):
            ...
            for user_id, item in hists_seed_by_user.items():
                hist_train_and_valid, hist_test_x, hist_test_y, hist_train_range = item
                ...
                hists_inner_seed_by_user[user_id] = [hist_train, hist_valid, hist_test_x, hist_test_y, hist_train_range]
            ...
            for user_id, item in hists_inner_seed_by_user.items():
                ...
                hist_train, hist_valid, hist_test_x, hist_test_y, hist_train_range = item
                ...
                # set range of hist_train in h2_train
                hist = Models.History(hist_train_x, hist_train_y)
                hist.set_range(hist_train_range, len(h2_train))
                    ...
                    for model_name in model_names:
                        if model_name == 'no hist':
                            ...
                            model = Models.get_model(
                                ...
                                diss_weight=weight, hist=hist, ccp_alpha=ccp_alpha, max_depth=max_depth,
                                ridge_alpha=ridge_alpha)
                        models.append(model)
                ...
                for metric_idx, metric in enumerate(metrics):
                    for subset_idx, subset in enumerate(['valid', 'test']):
