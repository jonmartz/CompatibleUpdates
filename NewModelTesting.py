...
for user_col in user_cols:
    ...
    for seed in seeds:
        ...
        X = scaler.transform(h2_train.drop(columns=[target_col]))
        Y = labelizer.transform(h2_train[[target_col]])
        Xs_by_seed += [X]
        Ys_by_seed += [Y]

        # train h1
        h1 = Models.NeuralNetwork(X, Y, h1_train_len, h1_epochs, batch_size, layers, 0.02,
        ...
        if make_plots:
            ...
            plt.title(
                'seed=' + str(seed) + ' train=' + str(h1_train_len) + ' test=' + str(h2_train_len - h1_train_len) +
        ...
        if not only_train_h1 and 'no hist' in models_to_test:
            ...
            for diss_weight in diss_weights:
                ...
                print('\tdiss weight ' + str(len(h2s_no_hist) + 1) + "/" + str(len(diss_weights)) +
    ...
    for user_id in user_ids:
        ...
        for seed_idx in range(len(seeds)):
            ...
            print('\n' + str(iteration) + '/' + str(iterations) + ' ' + user_col + '=' + str(user_id) +
                  ' len=' + str(history_len) + ' seed=' + str(seed))
                    ...
                    if model_name == 'no hist':
                        ...
                        if 'hybrid' in model_name:
                            ...
                            h2 = Models.NeuralNetwork(X, Y, h2_train_len, h2_epochs, batch_size, layers, 0.02, weight,
                                                      h1, 'D', history=hist, use_history=True, model_name=model_name,
...
# todo: train larger lambda (smaller train set) with more epochs
# todo: implement ensemble method
# todo: L0 model with nn likelihood
# todo: L3 model with L + lambda * (normal diss + hist diss)
