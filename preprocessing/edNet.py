            ...
            for index, row in df_user.iterrows():
                ...
                if action == 'enter':  # entered either bundle, explanation or lecture
                    ...
                    if len(questions_in_bundle) > 0:
                        ...
                        user_bundle['error_rate'].append(
                            1 - np.mean([q['answers'][-1] for i, q in questions_in_bundle.items()]))
                        for question_id, question_in_bundle in questions_in_bundle.items():
                            is_correct = question_in_bundle['answers'][-1]
                            user_question_answers = get_or_init_value_from_dict(user_questions, question_id, [])
                            ...
                            for skill_id in skill_ids:
                                user_skill = get_or_init_value_from_dict(
                                    user_skills, skill_id, {'answers': [], 'explanations': [], 'lectures': []})
                                user_skill['answers'].append(is_correct)
                            ...
                            writer.writerow([
                                ...
                                np.mean([1 - np.mean(i['answers']) for i in question_skills]),  # user_skill_error_rate
...
users = [503066, 480292, 295872, 357194, 759070, 332183, 342307, 357889, 773623, 533110]
# write_bkt_matrix(users_dir, users, dataset_dir)
# write_error_rates(users_dir, users, dataset_dir)
# write_bundle_content(dataset_dir)
